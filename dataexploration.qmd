...
data_individual = data |>
  ...
  group_by(SERIAL) |>
    ...
    filter(any(SPEARNWEEK <= topcode_earnweek 
               & EARNWEEK <= topcode_earnweek 
               & SPEARNWEEK > 0 
               & EARNWEEK > 0, na.rm = TRUE)) |>
    # think i will need people with regular working hours
    filter(any(UHRSWORKT < varying_workhours 
               & SPUSUALHRS < varying_workhours
               & UHRSWORKT > 0 
               & SPUSUALHRS > 0, na.rm = TRUE)) |>
  ungroup() |>
  ...
  # construct hourly earnings using weekly hrs worked and weekly earnings
  # respondent hourly wage and # spouse hourly wage
  mutate(
    respondent_hrly_wage = case_when(
      EARNWEEK < 99999.99 & UHRSWORKT < 995 ~ EARNWEEK / UHRSWORKT,
      TRUE ~ NA_real_),
    spouse_hrly_wage = case_when(
      SPEARNWEEK < 99999.99 & SPUSUALHRS < 995 ~ SPEARNWEEK / SPUSUALHRS,
      TRUE ~ NA_real_
    )) |>
  
  # construct income contributions of spouse and respondent to hh
  mutate(
    respondent_income_contrib = case_when(
      EARNWEEK < 99999.99 & UHRSWORKT < 995 ~ EARNWEEK / (EARNWEEK+SPEARNWEEK),
      TRUE ~ NA_real_),
    spouse_income_contrib = case_when(
      SPEARNWEEK < 99999.99 & SPUSUALHRS < 995 ~ SPEARNWEEK / (EARNWEEK+SPEARNWEEK),
      TRUE ~ NA_real_
    ))
...
Hours spent on private leisure
