...
hh = final_individual_data |>
  ...
  summarise(
    ...
    # include deviations
    dev_wage_f_only = max(dev_wage_f, na.rm=TRUE),
    dev_wage_m_only = max(dev_wage_m, na.rm=TRUE),
    dev_educ_f_only = max(dev_educ_f, na.rm=TRUE),
    dev_educ_m_only = max(dev_educ_m, na.rm=TRUE),
    dev_wage_f_all = max(dev_wage_f, na.rm=TRUE),
    dev_wage_m_all = max(dev_wage_m, na.rm=TRUE),
    dev_educ_f_all = max(dev_educ_f, na.rm=TRUE),
    dev_educ_m_all = max(dev_educ_m, na.rm=TRUE),
    dev_avg_age = max(dev_avgage, na.rm=TRUE),
    dev_age_gap = max(dev_agegap, na.rm=TRUE),
    
    # interaction terms for the SUR
    Bx_dev_wage_f_only = max(Bx_dev_wage_f, na.rm=TRUE),
    Bx_dev_wage_m_only = max(Bx_dev_wage_m, na.rm=TRUE),
    Bx_dev_educ_f_only = max(Bx_dev_educ_f, na.rm=TRUE),
    Bx_dev_educ_m_only = max(Bx_dev_educ_m, na.rm=TRUE),
    Bx_dev_wage_f_all = max(Bx_dev_wage_f, na.rm=TRUE),
    Bx_dev_wage_m_all = max(Bx_dev_wage_m, na.rm=TRUE),
    Bx_dev_educ_f_all = max(Bx_dev_educ_f, na.rm=TRUE),
    Bx_dev_educ_m_all = max(Bx_dev_educ_m, na.rm=TRUE),
