...
activity_summaries_2015 = data_activities_2015 |>
  ...
  mutate(
    ...
    activity2_is_otherdomestic = whatdoing %in% otherdomestic_actlines,
    ...
    activity3_is_otherdomestic = whatdoing %in% otherdomestic_actlines,
    ...
    activity4_is_otherdomestic = whatdoing %in% otherdomestic_actlines,
    ...
    # general: is domestic (aside from childcare)?
    ...
    # general: is domestic (all domestic work)?
  ...
  summarise(
    ...
    total_otherdomestic_nospouse = sum(eptime[domestic_nospouse], na.rm = TRUE) / 60,
...
parents_est_data_2015 = data_working_parents_2015 |>
  ...
  mutate(
    ...
    private_leisure_exp_f = private_leisure_exp_f * deflator_2014,
...
nonparents_est_data_2015 = data_working_nonparents_2015 |>
  ...
  pivot_wider(
    # keep all the household-level stuff: kids, region, serial
    id_cols = c(serial, is_weekend, dgorpaf, Income),
    names_from = sex_tag,
    values_from = all_of(vars_to_suffix),
    names_sep = "_") |>
  inner_join(regionalwealth_2014, by = c("dgorpaf")) |>
